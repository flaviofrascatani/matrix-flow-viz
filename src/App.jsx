...
function buildGraph(steps, nHeads, nLayers) {
  ...
  const qkvGap = 10;        // vertical gap between Q, K, V within a head
  const headGap = 30;       // vertical gap between heads
  const colW = W + gx;

  const headBandH = H * 3 + qkvGap * 2;   // height of Q+K+V stack
  const totalBandH = nHeads * headBandH + (nHeads - 1) * headGap;
  const topPad = 30;

  // Center Y = middle of all head bands
  const centerY = topPad + totalBandH / 2 - H / 2;
  ...
  for (let L = 0; L < nLayers; L++) {
    ...
    // ── Column X positions within this layer ──
    const qkvX   = lx;
    const scoresX = lx + colW;
    const smX     = scoresX + colW;
    const avX     = smX + colW;

    const qkv = [];       // [h][0=Q,1=K,2=V] node refs
    const attnVNodes = []; // attn·V node refs per head

    for (let h = 0; h < nHeads; h++) {
      // Top of this head's band
      const bandTop = topPad + h * (headBandH + headGap);
      // Center Y of this head's band (aligned with K node)
      const headCY = bandTop + H + qkvGap; // K's top = center row

      // ── Q, K, V (stacked vertically, left column) ──
      const hNodes = [];
      for (let q = 0; q < 3; q++) {
        const s = steps[si++];
        const ny = bandTop + q * (H + qkvGap);
        nodes.push({ s, x: qkvX, y: ny, id: s.id });
        hNodes.push({ id: s.id, x: qkvX, y: ny });
        // Edge from previous layer output → each Q/K/V
        edges.push({ from: prevOut, to: s.id, color: PC.qkv });
      }
      qkv.push(hNodes);

      // ── Scores: Q·Kᵀ/√d (aligned with K = center of head band) ──
      const sc = steps[si++];
      nodes.push({ s: sc, x: scoresX, y: headCY, id: sc.id });
      // Q → Scores
      edges.push({ from: hNodes[0].id, to: sc.id, color: PC.scores });
      // K → Scores
      edges.push({ from: hNodes[1].id, to: sc.id, color: PC.scores });

      // ── Softmax ──
      const sm = steps[si++];
      nodes.push({ s: sm, x: smX, y: headCY, id: sm.id });
      edges.push({ from: sc.id, to: sm.id, color: PC.softmax });

      // ── Attn·V ──
      const av = steps[si++];
      nodes.push({ s: av, x: avX, y: headCY, id: av.id });
      // Softmax → Attn·V
      edges.push({ from: sm.id, to: av.id, color: PC.context });
      // V → Attn·V
      edges.push({ from: hNodes[2].id, to: av.id, color: PC.context });

      attnVNodes.push({ id: av.id, x: avX, y: headCY });
    }

    // ── Concat (skip as visible node — merge into +Res+LN) ──
    // We still consume the concat step from the array but don't show it separately
    const ccStep = steps[si++];

    // ── +Res+LN (Post-MHA) — single merge node at center ──
    const resX = avX + colW;
    const pm = steps[si++];
    // Override the label to make it clearer
    const pmNode = { ...pm, label: '+Res+LN' };
    nodes.push({ s: pmNode, x: resX, y: centerY, id: pm.id });
    // All Attn·V outputs → +Res+LN
    attnVNodes.forEach(n => edges.push({ from: n.id, to: pm.id, color: PC['post-attn'] }));
    // Residual skip (dashed green)
    edges.push({ from: prevOut, to: pm.id, color: PC.residual, dashed: true });

    // ── FFN Up ──
    const fuX = resX + colW;
    const fu = steps[si++];
    nodes.push({ s: fu, x: fuX, y: centerY, id: fu.id });
    edges.push({ from: pm.id, to: fu.id, color: PC['ffn-up'] });

    // ── GELU ──
    const geX = fuX + colW;
    const ge = steps[si++];
    nodes.push({ s: ge, x: geX, y: centerY, id: ge.id });
    edges.push({ from: fu.id, to: ge.id, color: PC.gelu });

    // ── FFN Down (+Res+LN) ──
    const fdX = geX + colW;
    const fd = steps[si++];
    const fdNode = { ...fd, label: 'FFN ↓+Res+LN' };
    nodes.push({ s: fdNode, x: fdX, y: centerY, id: fd.id });
    edges.push({ from: ge.id, to: fd.id, color: PC['ffn-down'] });
    // FFN residual skip (dashed green)
    edges.push({ from: pm.id, to: fd.id, color: PC.residual, dashed: true });
